...
weekday_mean_variance = pd.read_csv("https://raw.githubusercontent.com/stho382/ENGSCI_Group_6_OR/main/Data/weekday_mean_demand_and_variance.csv", usecols= range(0,3))
weekend_mean_variance = pd.read_csv("https://raw.githubusercontent.com/stho382/ENGSCI_Group_6_OR/main/Data/weekend_mean_demand_and_variance.csv", usecols= range(0,3))
...
def weekday_sim():
    ...
    sum_simulation_costs_weekday = [0] * 2000
    median_extra_trucks_weekday = [0] * 2000

    # Simulating routes
    for i in range(len(sum_simulation_costs_weekday)):
        # Using bootstrap method to repeatedly resample demands for each store
        store_vars = {}
        for j in range(len(weekday_mean_variance['Store'].values)):
            store_name = weekday_mean_variance['Store'].values[j] # store
            mu = weekday_mean_variance['Weekday_mean'].values[j] # mean
            sigma = np.sqrt(weekday_mean_variance['Weekday_variance'].values[j]) # standard deviation
            ...
            store_vars.update({store_name : results})
        
        # finding the routes for the simulation
        good_path_vars = {}
        extra_trucks_used_weekday = 0
        ...
        for route in allweekdayroutes:
            ...
            extra_trucks_used_weekday += no_of_trucks
        ...
        for paths in good_path_vars:
            route_cost.update({ paths : find_my_cost(list(paths), store_vars)})
        
        sum_simulation_costs_weekday[i] = sum(list(route_cost.values()))
        median_extra_trucks_weekday[i] = extra_trucks_used_weekday


    print("No. of trucks used in total: " + str(num_weekday + math.ceil(statistics.median(median_extra_trucks_weekday))))


    # Average cost for weekdays
    print("Average cost for weekdays: $" + str(statistics.median(sum_simulation_costs_weekday)))


    # One sample t-test, with H0 = expected cost for weekdays.
    print("One sample T-test for weekday: " + str(stats.ttest_1samp(sum_simulation_costs_weekday, statistics.median(sum_simulation_costs_weekday))))


    # Percentile interval
    sum_simulation_costs_weekday.sort()
    lower_bound = sum_simulation_costs_weekday[25]
    upper_bound = sum_simulation_costs_weekday[1975]

    print("lower bound: " + str(lower_bound))
    print("upper bound: " + str(upper_bound))

    # show the plot
    sns.distplot(sum_simulation_costs_weekday)
...
def weekend_sim():
    # Generating the max costs times for weekdays
    sum_simulation_costs_weekend = [0] * 2000
    median_extra_trucks_weekend = [0] * 2000

    for i in range(len(sum_simulation_costs_weekend)):
        # Using bootstrap method to repeatedly resample demands for each store
        store_vars = {}
        for j in range(len(weekend_mean_variance['Store'].values)):
            store_name = weekend_mean_variance['Store'].values[j]
            mu = weekend_mean_variance['Weekend_mean'].values[j]
            sigma = np.sqrt(weekend_mean_variance['Weekend_variance'].values[j])
            # Assuming normal distribution and bootstrapping repeatedly
            results = statistics.median(stats.norm.rvs(loc = mu, scale = sigma, size=50))
        ...
        sum_simulation_costs_weekend[i] = sum(list(route_cost.values()))
        median_extra_trucks_weekend[i] = extra_trucks_used_weekend


    print("No. of trucks used in total: " + str(num_weekend + math.ceil(statistics.median(median_extra_trucks_weekend))))


    # Average cost for weekdays
    print("Average cost for weekend: $" + str(statistics.median(sum_simulation_costs_weekend)))


    # One sample t-test, with H0 = expected cost for weekdays.
    print("One sample T-test for weekdend: " + str(stats.ttest_1samp(sum_simulation_costs_weekend, statistics.median(sum_simulation_costs_weekend))))


    # Percentile interval
    sum_simulation_costs_weekend.sort()
    lower_bound = sum_simulation_costs_weekend[25]
    upper_bound = sum_simulation_costs_weekend[1975]
